...
def demo_complete_name_processing_workflow():
    ...
    print("\nProcessed Results:")
    result_cols = [
        'candidate_id', 'raw_name', 'parsed_first_name', 'parsed_last_name',
...
def demo_multi_format_export():
    ...
    formats = [
        ...
        ("JSON", "processed_names.json", lambda f: df.to_json(f, orient="records", indent=2)),
    ]

    # Try Parquet if available
    try:
        import pyarrow  # noqa: F401
        formats.append(("Parquet", "processed_names.parquet", lambda f: df.to_parquet(f, index=False)))
    except ImportError:
