...
def clean_names_cmd(
    ...
    try:
        with console.status("[bold green]Cleaning names...") as status:
...
def merge_supplementary_data_cmd(
    ...
    nickname_file: Optional[Path] = typer.Option(
        None, "--nickname-file", 
...
def preprocess_cmd(
    ...
    drop_patterns_file: Optional[Path] = typer.Option(
        None, "--drop-patterns", "-d",
        help="File with patterns to drop (default: drop_patterns.txt)"
    ),
    edit_lengths: Optional[List[int]] = typer.Option(
        None, "--edit-lengths", "-e",
...
def split_corpus_cmd(
    ...
    output_pattern: Optional[str] = typer.Option(
        None, "--output", "-o",
...
def search_cmd(
    ...
    try:
        with Progress() as progress:
            task = progress.add_task("[bold green]Searching names...", total=None)
