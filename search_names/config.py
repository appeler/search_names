...
class NameCleaningConfig:
    ...
    default_patterns: List[str] = field(default_factory=lambda: [
        "FirstName LastName", 
        "NickName LastName", 
        "Prefix LastName"
    ])
    roman_numerals: List[str] = field(default_factory=lambda: [
...
class ConfigManager:
    ...
    def save_config(self, config: Config, path: Optional[Union[str, Path]] = None) -> None:
    ...
    def _dict_to_config(self, config_dict: Dict[str, Any]) -> Config:
        ...
        if 'name_cleaning' in config_dict:
            config.name_cleaning = dict_to_dataclass(NameCleaningConfig, config_dict['name_cleaning'])
        if 'text_processing' in config_dict:
            config.text_processing = dict_to_dataclass(TextProcessingConfig, config_dict['text_processing'])
...
def create_sample_config(path: Union[str, Path] = "search_names_sample.yaml") -> None:
